...
class Receiver:
    def __init__(self, config, debug_data=None):
        ...
        self.cnt_length = config.data_cnt_length
        ...
        self.data_bit_map = np.zeros((26215,), dtype=np.uint8)
        self.data_buffer = [""] * 26215
    ...
    def checkCRC(self, decode_FIFO_power_bit):
        ...
        if not (dec2arr(int(crc_check, 16), self.crc_length) == decode_FIFO_power_bit[
            ...
            for k in range(self.id_length):
                index = index + decode_FIFO_power_bit[curr_position+k] * (2 ** (self.id_length - k - 1))
            curr_position += self.id_length

            if index != self.expect_id:
                return index

            seq_num = 0
            for k in range(self.seq_num_length):
                seq_num = seq_num + decode_FIFO_power_bit[curr_position+k] * (2 ** (self.seq_num_length - k - 1))
            curr_position += self.seq_num_length

            total_seq = 0
            for k in range(self.total_seq_length):
                total_seq = total_seq + decode_FIFO_power_bit[curr_position+k] * (2 ** (self.total_seq_length - k - 1))
            curr_position += self.total_seq_length

            type = 0
            for k in range(self.type_length):
                type = type + decode_FIFO_power_bit[curr_position+k] * (2 ** (self.type_length - k - 1))
            curr_position += self.type_length

            if type != self.expect_type:
                return index
            else:
                self.type = type
            ...
            for k in range(self.cnt_length):
                cnt = cnt + decode_FIFO_power_bit[curr_position+k] * (2 ** (self.cnt_length - k - 1))
            curr_position += self.cnt_length

            ip_list = []
            for bin_seg in re.findall(r'.{8}', arr2str(decode_FIFO_power_bit[curr_position:curr_position+self.ip_length])):
                str_seg = str(int(bin_seg, 2))
                ip_list.append(str_seg)
            ip = ".".join(ip_list)
            curr_position += self.ip_length

            port = arr2dec(decode_FIFO_power_bit[curr_position:curr_position+self.port_length]); curr_position += self.port_length
            proto = arr2dec(decode_FIFO_power_bit[curr_position:self.header_length]); curr_position = self.header_length
            self.proto = proto

            if 0 <= seq_num < total_seq and self.data_bit_map[seq_num] != 1:
                # print("Correct ID:", index, "\t IP:", ip, "\t port:", port, "\t payload:", cnt//8, "bytes")
                data_str = arr2str(decode_FIFO_power_bit[self.header_length:self.header_length+cnt])
                self.data_buffer[seq_num] = data_str
                self.data_bit_map[seq_num] = 1

            if np.sum(self.data_bit_map[:total_seq]) == total_seq:
                # print("All data correctly received!")
                self.data_buffer = self.data_buffer[:total_seq]
    ...
    def receive(self, expect_id, expect_type):
        ...
        return self.type, self.proto, self.data_buffer
